...
Это обновление улучшает отображение и продолжение диалогов, а также работу на мобильных устройствах.

- **Понятнее ход ответа**: ответ появляется постепенно, а промежуточный текст, работа инструментов и итоговый ответ показываются в фактическом порядке. После завершения детали процесса можно свернуть.
- **Надежнее обратная связь о завершении**: анимация текущей активности больше не зависит от старых неудачных операций, а завершенная задача сообщает о внесенных изменениях.
- **Продолжение после сбоя сети**: после временного обрыва сети диалог можно продолжить с сохранением уже выполненных записей.
- **Точнее продолжение задач и подтверждение**: приостановленную задачу в длинном диалоге можно продолжить. Нельзя отправить пустой ответ на вопрос, и отсутствие ответа не считается подтверждением.
- **Удобнее касания и карточка «Сегодня»**: при медленном касании экран реже неожиданно переходит в конец содержимого, а кнопки карточки «Сегодня» лучше помещаются на узких экранах.
- **Улучшены Skills на мобильных устройствах**: разбор встроенных Skills и связанные с ними сценарии взаимодействия доработаны для мобильного использования.
