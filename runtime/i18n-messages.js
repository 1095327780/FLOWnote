  ...
  "zh-CN": {
    ...
    settings: {
      ...
      agent: {
        ...
        testNameForProvider: "测试 {provider} 连接",
        testDescForProvider: "测试当前电脑端服务商配置：{provider} / {model}。会验证 URL、Key 和模型是否可用。",
        modelDescOllama: "选择当前本机 Ollama 已安装的模型。插件会从 localhost:11434/v1/models 读取列表；点击右侧刷新可重新读取。",
        modelRefreshTooltipOllama: "从本机 localhost:11434/v1/models 重新读取已安装模型。",
        modelMissingInOllamaList: "{model}（未在当前 Ollama 列表中）",
        modelDescFetchedOllama: "模型列表已于 {min} 分钟前从本机 Ollama 刷新。",
        modelRefreshFallbackStatic: "模型列表接口暂不可用，已保留内置模型列表。",
        ollamaLocalNoticeTitle: "本地 Ollama 模型建议",
        ollamaLocalNoticeBody: "FLOWnote 会发送技能说明、工具列表和笔记上下文。本地模型建议使用上下文窗口 16K tokens 以上、参数规模 7B/8B 以上的工具调用模型，更推荐 14B 以上。规格太小可能只会回复“我会调用工具”，但实际不会执行技能或写入笔记。",
      },
      basic: {
  ...
  en: {
    ...
    settings: {
      ...
      agent: {
        ...
        testNameForProvider: "Test {provider} connection",
        testDescForProvider: "Tests the current desktop provider config: {provider} / {model}. It verifies the URL, key, and model.",
        modelDescOllama: "Choose a model installed in the current local Ollama. FLOWnote reads localhost:11434/v1/models; refresh reads it again.",
        modelRefreshTooltipOllama: "Read installed models from local localhost:11434/v1/models.",
        modelMissingInOllamaList: "{model} (not in current Ollama list)",
        modelDescFetchedOllama: "Model list refreshed from local Ollama {min} minutes ago.",
        modelRefreshFallbackStatic: "The model-list endpoint is unavailable, so the built-in model list was kept.",
        ollamaLocalNoticeTitle: "Local Ollama model recommendation",
        ollamaLocalNoticeBody: "FLOWnote sends skill instructions, tool schemas, and note context. For local models, use a tool-calling model with at least a 16K-token context window and 7B/8B parameters; 14B+ is recommended. Smaller models may say they will call a tool without actually running the skill or writing notes.",
      },
      basic: {
